...
child [concept color = gray]{ node {User}
  child { node {Seller}
    child { node {Provide Resources}}
    child { node {Schedule Orders}}
    child { node {Outsource Orders}}
    child { node {Evaluate Buyer}}
  }
  child
  child { node {Buyer}
    child { node {Submit Order}}
    child { node {Choose Seller}}
    child { node {Evaluate Seller}}
  }
  }
child [concept color = gray]{ node {Evolve}
