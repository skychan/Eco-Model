...
Apart from $mr_k$ respond to $t_{ij}$ as we explained above, there are 2 other cases in incubation mode:
...
\suspend{asparaenum}
Select $mr_k$ for $sc_l$ when $\mathcal{B}_{l,\tau}\subset\mathcal{A}_l$
...
Select $ms_l$ for $t_{ij}$ when $\mathcal{B}_{ij,\tau}\subset\mathcal{A}_{ij},|\mathcal{S}_{ij,\tau}|>0$
...
Select machine for $t_{ij}$ when $\mathcal{B}_{ij,\tau} = \mathcal{A}_{ij},\abs{\mathcal{S}_{ij,\tau}}>0$.
