...
\begin{tabularx}{\textwidth}{|lX|}
    ...
    \multicolumn{2}{|l|}{} \\
    $\mathcal{T}_i$ & Order that comes with demander, it can be quired by $F(\cdot)$ \\
    ...
    $mr_k$ & Resource that comes with provider, it can be quired by $F(\cdot)$ \\
    ...
    $\mathcal{A}_{ij}$ & Subset of resource type required by $t_{ij}$\\ %, $\mathcal{A}_{ij}\subset\mathcal{A}$\\
    ...
    $\mathcal{A}_l$ & Subset of resource type required by $sc_l$ or provided by $ms_l$ \\ %, $\mathcal{A}_l\subset\mathcal{A}$\\
...
\end{tabularx}
\end{table}
Since demander and provider arrives successively, there is no upper bound of the subscripts
$(i,j,k,l,\alpha)$.
...
The life cycle management of resource contains its vicissitude, which is the basis of the following extended modes \autoref{fig:modes}.
\begin{figure}[htbp]
    \centering
    ...
    \label{fig:modes}
\end{figure}
Incubation mode and outsourcing mode are options for provider, while metabolism mode is option for platform operator. Outsourcing mode can only be applied if incubation mode is applied because only the task that processed by service can be outsourced.
...
A demander makes decision about resource selection from $\mathcal{R}_{ij,\tau}$ when $\mathcal{B}_{ij,\tau} = \mathcal{A}_{ij}$. Without loss of generality, we suppose at time $\tau \ge r_i$,
...
% \autoref{eq:tempqualitystart}--\ref{eq:tempqualityend} are the virtual quality value calculation procedure.
...
Metabolism mode is one extension for the original mode to control the number of entity in the system by both restrict the arrival and eliminate the current entities. As metabolism part shown in \autoref{fig:modes},
% \begin{figure}[htbp]
%     \centering
...
\begin{figure}[htbp]
	\centering
	\large
	\resizebox{.9\textwidth}{!}{\input{figures/scheduleChart}}
